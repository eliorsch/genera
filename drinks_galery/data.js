...
let drinksList;
	...
	let buildGallery = (list, parent) => {
		...
		for (let i = 0; i < list.length; i++) {
			...
			buildElement("p", "name", null, list[i].strDrink, null, child, null);

			child = buildElement("div", "picture", null, null, null, card, null);
			buildElement("img", null, null, null, null, child, list[i].strDrinkThumb, "onclick", bla);

			child = buildElement("div", "details", null, null, null, card, null);
			child = buildElement("ul", null, null, null, null, child, null);
			for (let k = 1; list[i]["strIngredient" + k] && k<10; k++) {
				let amount = '';
				if (list[i]["strMeasure" + k]) amount = list[i]["strMeasure" + k];
				buildElement("li", null, null, amount + list[i]["strIngredient" + k], null, child, null);
			}
...
function viewGalery(){
	...
	buildGallery(drinksList, container);
}
function selectDrink() {
	let input = document.querySelector("#selectDrinkINPUT");
	console.log("https://www.thecocktaildb.com/api/json/v1/1/search.php?s=" + input.value);
	console.log(typeof("https://www.thecocktaildb.com/api/json/v1/1/search.php?s=" + input.value));
	axios.get("https://www.thecocktaildb.com/api/json/v1/1/search.php?s=" + input.value).then(data => {console.log(data.data);drinksList = data.data.drinks; viewGalery();});
